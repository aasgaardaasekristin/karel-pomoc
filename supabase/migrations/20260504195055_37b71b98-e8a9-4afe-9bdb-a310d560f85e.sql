...
CREATE OR REPLACE FUNCTION public.invoke_p23_canary_generic(p_function text, p_body jsonb)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public','vault','net'
...
DECLARE
  ...
  v_request_id bigint;
BEGIN
  IF p_function !~ '^karel-' THEN
    RAISE EXCEPTION 'function must start with karel-';
  END IF;
  SELECT decrypted_secret INTO v_secret
  FROM vault.decrypted_secrets WHERE name='KAREL_CRON_SECRET' LIMIT 1;
  IF COALESCE(v_secret,'') = '' THEN
