ALTER TABLE public.did_team_deliberations
  DROP CONSTRAINT IF EXISTS did_team_deliberations_status_chk;

ALTER TABLE public.did_team_deliberations
  ADD CONSTRAINT did_team_deliberations_status_chk
  CHECK (status IN ('draft','active','awaiting_signoff','approved','in_revision','closed','archived'));

CREATE TABLE IF NOT EXISTS public.did_daily_session_start_audit (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  deliberation_id uuid,
  plan_id uuid,
  started_by uuid,
  sync_source text NOT NULL DEFAULT 'sync_and_start_approved_daily_plan',
  approval_hashes jsonb NOT NULL DEFAULT '{}'::jsonb,
...
ALTER TABLE public.did_daily_session_start_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own daily session start audit"
ON public.did_daily_session_start_audit
FOR SELECT TO authenticated
USING (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_did_daily_session_start_audit_user_created
ON public.did_daily_session_start_audit(user_id, created_at DESC);
...
BEGIN
  ...
  IF existing_markdown_hash IS NOT NULL AND existing_markdown_hash <> md5(COALESCE(plan_row.plan_markdown, '')) THEN
    INSERT INTO public.did_daily_session_start_audit(user_id, deliberation_id, plan_id, started_by, approval_hashes, result, error_code, message)
    VALUES (p_user_id, delib.id, plan_row.id, p_user_id, audit_hashes || jsonb_build_object('existing_plan_markdown_hash', existing_markdown_hash, 'current_plan_markdown_hash', md5(COALESCE(plan_row.plan_markdown, ''))), 'blocked', 'plan_markdown_hash_mismatch', 'Hash propsaného denního plánu nesedí.');
