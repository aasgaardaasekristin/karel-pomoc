 ...
 *
 * Inspirace: Jung asociační experiment, Machover DAP, Koch Baum, Buck HTP,
 * Burns KFD, CAT/TAT, Lowenfeld sandtray, somatic body map, EMDR safe place.
 *
 * NEsimulujeme reálné testy 1:1 (na to nejsou data ani validace),
...
export type Playbook = {
  ...
  trauma_response_protocol: {
    ...
    do_not_repeat_stimulus: boolean;
    grounding_script: string;
  };
  closure_protocol: {
    reproduction_check: string | null;
...
export const playbook_sandtray_lowenfeld: Playbook = {
  ...
  pre_session_setup: {
    supplies: [
      "set figurek (lidi, zvířata, vozidla, domy, přírodní objekty, stinné figurky)",
      "podložka / pískoviště",
      "audio záznam",
      "fotoaparát",
    ],
...
export function renderPlaybookForPrompt(p: Playbook, plannedSteps?: string[]): string {
  const steps = plannedSteps && plannedSteps.length
    ? plannedSteps
    : (p.step_protocol.planned_steps || []);

  const lines: string[] = [];
  lines.push("═══ KLINICKÝ PLAYBOOK (POVINNÝ) ═══");
  lines.push(`METODA: ${p.method_label}`);
  lines.push(`ZDROJ: ${p.source_refs.join("; ")}`);
  lines.push("");
  lines.push("▼ PRE-SESSION SETUP (musíš nejdřív vysvětlit terapeutce, NEŽ dáš první stimul)");
  lines.push(`Pomůcky: ${p.pre_session_setup.supplies.join(", ")}`);
  lines.push(`Místnost: ${p.pre_session_setup.room}`);
  lines.push(`Pozice terapeutky: ${p.pre_session_setup.therapist_position}`);
  lines.push(`Pozice dítěte: ${p.pre_session_setup.child_position}`);
  lines.push("Co PŘESNĚ říct dítěti na začátku:");
  lines.push(p.pre_session_setup.what_to_say_first);
  lines.push("Co NESMÍŠ dělat / říkat:");
  for (const s of p.pre_session_setup.what_NOT_to_say) lines.push(`  - ${s}`);
  lines.push("CO MUSÍ TERAPEUTKA POVINNĚ ZAZNAMENÁVAT KAŽDÝ TURN:");
  for (const s of p.pre_session_setup.measurements_required) lines.push(`  • ${s}`);
  lines.push("");
  lines.push(`▼ STEP PROTOCOL (typ: ${p.step_protocol.kind})`);
  if (steps.length) {
    lines.push(`Plánované stimuly (${steps.length}):`);
    steps.forEach((s, i) => lines.push(`  ${i + 1}. ${s}`));
  }
  lines.push(`Instrukce na každý krok: ${p.step_protocol.instruction}`);
  lines.push(`Co zaznamenat: ${p.step_protocol.what_to_record.join(", ")}`);
  lines.push("RED FLAGS (kdy přerušit a přejít do trauma_pause):");
  for (const s of p.step_protocol.red_flags) lines.push(`  ⚠ ${s}`);
  lines.push("");
  lines.push("▼ TRAUMA RESPONSE PROTOCOL");
  lines.push(`Příznaky: ${p.trauma_response_protocol.signs.join("; ")}`);
  lines.push(
    `DO NOT REPEAT STIMULUS: ${
      p.trauma_response_protocol.do_not_repeat_stimulus
        ? "ANO — NIKDY neopakuj stejný stimul po traumatické reakci"
        : "ne"
    }`,
  );
  lines.push("Okamžité akce:");
  for (const s of p.trauma_response_protocol.immediate_actions) lines.push(`  → ${s}`);
  lines.push("Grounding skript pro dítě:");
  lines.push(p.trauma_response_protocol.grounding_script);
  lines.push("");
  lines.push("▼ CLOSURE");
  lines.push(
    p.closure_protocol.reproduction_check
      ? `Reprodukční zkouška: ${p.closure_protocol.reproduction_check}`
      : "(bez reprodukční zkoušky)",
  );
  lines.push("Debrief otázky:");
  for (const s of p.closure_protocol.debrief_questions) lines.push(`  • ${s}`);
  lines.push(`Grounding po dokončení: ${p.closure_protocol.grounding}`);
  lines.push("");
  lines.push(`▼ POVINNÉ ARTEFAKTY: ${p.required_artifacts.join(", ")}`);
  lines.push("═══════════════════════════════════");
  return lines.join("\n");
}
