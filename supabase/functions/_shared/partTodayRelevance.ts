/**
 * P33.6 — Today-relevance gate for the primary "Dnešní část" suggestion.
 *
 * Pure function, no DB, no fetch. Caller passes already-collected context
 * (today_part_proposal, recent threads, session/playroom plans, live
 * progress, registry status, evidence_strength). Returns whether the
 * proposed part may be shown as today's primary suggestion plus a
 * normalized display name.
 *
 * Mirror: src/lib/partTodayRelevance.ts (1:1).
 */

export interface PartTodayRelevanceInput {
  proposed_part: string | null | undefined;
  briefing_date: string;          // YYYY-MM-DD (Prague)
  source_cycle_id: string | null | undefined;
  is_hypothesis_only: boolean;
  evidence_strength: "low" | "medium" | "high" | string | null | undefined;
  recent_thread_part_names: string[];        // last 24-72h
  todays_session_part_names: string[];       // session/playroom for today
  live_progress_part_names: string[];        // live_progress within 36h
  explicit_therapist_mentions: string[];     // pending question / answer mentions today
  registry_sleeping?: boolean;               // true when did_part_registry says dormant/sleeping
}
 ...
 * Returns null if name is empty/only-prefix.
 */
...
function nameInList(needle: string, list: string[]): boolean {
  ...
  for (const x of list) {
    ...
    // also accept normalized form match
...
export function isPartTodayRelevantForPrimarySuggestion(
  ...
  // Low support + hypothesis only AND no current evidence → reject as primary.
  if (input.is_hypothesis_only && evidence === "low" && !hasCurrentEvidence) {
    return {
      ok_for_primary_suggestion: false,
      reason: "low_support_hypothesis_without_current_evidence",
      display_name: display,
      confidence: "low",
    };
  }

  // Sleeping/dormant part needs CURRENT evidence to be primary.
  if (input.registry_sleeping === true && !hasCurrentEvidence) {
    return {
      ok_for_primary_suggestion: false,
      reason: "dormant_part_without_current_evidence",
      display_name: display,
      confidence: "low",
    };
  }
