...
const REAL_WORLD_FACT_RUNTIME_OVERREACH_PATTERN = /(identifikoval[aiy]?|identifikac[eií]|ztoto[zž]n[ěe]n[íi]|p[řr]evt[ěe]lil[ao]?|aktivoval[aoiy]?\s+v\s+nich|probudila\s+[^.!?\n]{0,80}\bvlnu\b|vy[čc]erpal[aoiy]?\s+[^.!?\n]{0,80}\b(?:d[ěe]ti|kluky|Tundrupka|je)\b|vy[žz]d[íi]mal|vlezl[ioy]?\s+do\s+toho|boj\s+o\s+[žz]ivot|uk[áa]zalo\s+to,?\s+jak\s+hluboko|touha\s+pom[áa]hat\s+a\s+zachra[nň]ovat|zasekl[aiy]?\s+v\s+tom|prolnul[ao]?|byl\s+Tim+m[iy]|byla\s+to\s+obrovsk[áa]\s+n[áa]lo[žz]|dozn[íi]v[áa].*v\s+d[ěe]tech|d[ěe]ti\s+to\s+c[íi]t[íi]|v\s+my[šs]lenk[áa]ch\s+po[řr][áa]d\s+u\s+toho|zachra[nň]ovali\s+sv[ěe]t)/i;
...
Nevyvodím z toho význam bez závěru bez vlastní reakce kluků. Dnes bych šel jemně: nejdřív ověřit, co o té situaci sami říkají, co cítí v těle, jestli jsou zahlcení a co by teď potřebovali, aby zůstali tady a v bezpečí.` : `Hani, rozumím. Je to skutečná aktuální situace a důležitý emoční kontext, ne automatický závěr o klucích.
