...
${existingCardsContext ? `\nEXISTUJÍCÍ KARTY (pro ověření existence částí):\n${existingCardsContext.slice(0, 3000)}` : ""}`,
      ...
      if (shouldSendEmails && resend && finalReportText) {
        try {
          ...
          // HANKA's report (professional team lead briefing)
          hankaHtml = "";
          try {
            const hankaRes = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
              method: "POST",
              ...
              body: JSON.stringify({
                model: "google/gemini-2.5-flash-lite",
                messages: [
                  { role: "system", content: `Jsi Karel – vedoucí terapeutického týmu pro DID případ. Vygeneruj profesionální DENNÍ BRIEFING pro Hanku.
...
POZOR: Pokud se o části pouze HOVOŘILO v terapeutickém rozhovoru, JASNĚ to rozliš – to NENÍ aktivita části!
...
Tón: profesionální, věcný, analytický. Vedoucí týmu, ne intimní dopis.
ŽÁDNÉ "milá Haničko", "lásko", "jsem tady pro tebe" – toto je pracovní briefing.
ADAPTIVNÍ STYL: Přizpůsob tón na základě motivačního profilu (pochvaly/termíny/instrukce).
