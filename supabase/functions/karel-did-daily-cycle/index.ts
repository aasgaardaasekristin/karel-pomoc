...
Pokud úkol visí 3+ dny, Karel automaticky eskaluje a v emailu svolá "poradu".`,
    ...
    if (validatedAnalysisText) {
      ...
      // ═══════════════════════════════════════════════════════════════════
      // P29B.2-CF: CENTRUM tail is FULLY DETACHED.
      // Previously this block ran [CENTRUM:...] processing, deterministic
      // CENTRUM fallback, [ACCOUNTABILITY] task escalation and the
      // 07_KNIHOVNA Drive read + AI + enqueue loop synchronously here
      // (~580 lines, ~150–250s wall-clock). The cycle frequently 504'd
      // before reaching the phase-job enqueues below.
      //
      // Now: persist a payload row to did_daily_cycle_phase_payloads,
      // store only a small payload_ref in context_data.phase4, and
      // enqueue a phase4_centrum_tail job. The detached worker invokes
      // _shared/dailyCyclePhase4CentrumTail.runPhase4CentrumTail.
      //
      // Forbidden in this region: any AI call, Drive list/read, KNIHOVNA
      // scan, CENTRUM block parsing loop, did_pending_drive_writes
      // insert, or other long-work for-loops.
      // ═══════════════════════════════════════════════════════════════════
      let centrumTailEnqueued = false;
      let centrumTailPayloadId: string | null = null;
      try {
        const validSourcesArr: string[] = [];
        for (const t of reportThreads) validSourcesArr.push(`${t.sub_mode}|${t.part_name}`);
        for (const c of reportConversations) validSourcesArr.push(`${c.sub_mode}|${c.label}`);
        if (recentHanaConversations.length > 0) validSourcesArr.push("hana|Hana");
        if (recentEpisodes.length > 0) validSourcesArr.push("episode|any");

        const registryParts = registryContext?.entries || [];
        const activePartsLite = registryParts
          .filter((e) => !isArchivedFromRegistry(e))
          .map((e) => ({
            id: String(e.id ?? ""),
            name: e.name ?? "",
            normalizedName: e.normalizedName ?? canonicalText(e.name ?? ""),
            cluster: e.cluster ?? "",
            age: e.age ?? "",
            status: e.status ?? "",
          }));
        const sleepingPartsLite = registryParts
          .filter((e) => isArchivedFromRegistry(e))
          .map((e) => ({
            id: String(e.id ?? ""),
            name: e.name ?? "",
        ...
        const payload = {
          validatedAnalysisText,
          analysisText,
          centrumFolderId: centrumFolderId ?? null,
          hasRecentActivity,
          dateStr: reportDatePrague,
          reportThreads: reportThreads.map((t) => ({ sub_mode: t.sub_mode, part_name: t.part_name })),
          reportConversations: reportConversations.map((c) => ({ sub_mode: c.sub_mode, label: c.label })),
          recentHanaConversations: recentHanaConversations.map(() => ({})),
          recentEpisodes: recentEpisodes.map(() => ({})),
          pendingTasks: pendingTasks ?? [],
          activeParts: activePartsLite,
          sleepingParts: sleepingPartsLite,
          existingCardsContext: (existingCardsContext || "").slice(0, 6000),
          blacklist: [],
          validSources: validSourcesArr,
          counts: {
            allRecentConversations: allRecentConversations.length,
            recentClientSessions: recentClientSessions.length,
        ...
        const { data: payloadRow, error: payloadErr } = await sb
        ...
        if (payloadErr || !payloadRow) {
          console.error("[PHASE_4_TAIL] payload insert failed:", payloadErr?.message);
        } else {
          ...
          const enqTail = await enqueuePhaseJob(sb as any, {
            cycle_id: cycle.id,
            user_id: resolvedUserId,
            phase_name: "phase4_centrum_tail",
            job_kind: "phase4_centrum_tail",
            input: {
              payload_ref: {
                payload_table: "did_daily_cycle_phase_payloads",
          ...
          centrumTailEnqueued = enqTail.ok;
          console.log(`[PHASE_4_TAIL] phase4_centrum_tail enqueued=${enqTail.ok} (${enqTail.reason ?? "ok"}) payload_id=${payloadRow.id}`);
        }
      } catch (tailErr: any) {
        console.error("[PHASE_4_TAIL] enqueue path failed (non-fatal):", tailErr?.message);
      }
