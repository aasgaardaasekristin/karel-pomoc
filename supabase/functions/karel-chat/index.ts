...
const SAFETY_PATTERNS: Record<string, { patterns: RegExp[]; severity: string }> = {
  suicidal_ideation: {
    patterns: [/nechci\s+žít/i, /chci\s+umřít/i, /zabij\s+m[ěe]/i, /skončit\s+se\s+vším/i, /nemá\s+to\s+(cenu|smysl)/i, /bez\s+m[ěe]\s+by\s+bylo\s+líp/i, /chci\s+zmizet\s+navždy/i, /kdybych\s+tu\s+nebyl/i, /přeji\s+si\s+smrt/i, /chci\s+se\s+zabít/i],
    severity: "critical",
  },
  self_harm: {
    patterns: [/řež[ue]\s+se/i, /ubližuj[ue]\s+si/i, /bolest\s+pomáhá/i, /chci\s+si\s+ublížit/i, /pálím\s+se/i, /škráb[ue]\s+se/i, /bouchám\s+hlavou/i, /rány\s+na\s+ruce/i, /krev\s+mi\s+pomáhá/i],
    severity: "high",
  },
  dissociative_crisis: {
    patterns: [/nevím\s+kde\s+jsem/i, /kdo\s+jsem/i, /nepoznávám/i, /všechno\s+je\s+neskutečn/i, /jsem\s+mimo\s+tělo/i, /nevidím\s+se/i, /ztráta\s+času/i, /probudil.*jsem\s+se.*nevím/i, /nemůžu\s+se\s+hýbat/i],
    severity: "high",
  },
  severe_distress: {
    patterns: [/nemůžu\s+dýchat/i, /panika/i, /hrůza/i, /třesu\s+se/i, /bojím\s+se\s+že\s+umřu/i, /nechci\s+být\s+sám/i, /prosím\s+pomoct/i, /nevydržím/i, /je\s+mi\s+hrozně/i, /chci\s+křičet/i],
    severity: "medium",
  },
  aggressive_outburst: {
    patterns: [/zabiju\s+(tě|ho|ji|je|vás)/i, /chci\s+někomu\s+ublížit/i, /zničím/i, /nenávidím\s+(tě|všechny)/i, /chci\s+rozbít/i, /ať\s+všichni\s+chcípnou/i],
    severity: "high",
  },
  abuse_disclosure: {
    patterns: [/někdo\s+mi\s+ubližuje/i, /bil\s+m[ěe]/i, /osahával/i, /znásilnil/i, /nesmím\s+říct/i, /tajemství.*bolí/i, /dělá\s+mi\s+věci/i],
    severity: "critical",
  },
  substance_mention: {
    patterns: [/vzal\s+jsem\s+(si\s+)?(prášky|léky|drogy)/i, /piju\s+hodně/i, /opil\s+jsem\s+se/i, /fetuju/i, /předávkoval/i],
    severity: "high",
  },
  runaway_intent: {
    patterns: [/utečou/i, /odejdu\s+z\s+domu/i, /schová[m]?\s+se/i, /zmizím/i, /nikdo\s+mě\s+nenajde/i],
    severity: "medium",
  },
};

async function checkSafetyPatterns(userText: string, partName: string): Promise<{ safe: boolean; alert: any | null }> {
  const detectedSignals: Array<{ type: string; severity: string; pattern: string }> = [];
  for (const [alertType, config] of Object.entries(SAFETY_PATTERNS)) {
    for (const pattern of config.patterns) {
      if (pattern.test(userText)) {
        detectedSignals.push({ type: alertType, severity: config.severity, pattern: pattern.source });
        break;
      }
    }
  }
  if (detectedSignals.length === 0) return { safe: true, alert: null };

  const severityOrder: Record<string, number> = { critical: 4, high: 3, medium: 2, low: 1 };
  const highestSeverity = detectedSignals.reduce((max, s) => (severityOrder[s.severity] || 0) > (severityOrder[max] || 0) ? s.severity : max, "low");
  const primaryType = detectedSignals[0].type;
  const actions: Record<string, string> = {
    critical: "OKAMŽITĚ kontaktovat Hanku. Zvážit krizovou intervenci.",
    high: "Upozornit Hanku co nejdříve. Karel by měl aktivovat bezpečnostní protokol.",
    medium: "Zaznamenat a sledovat. Informovat Hanku v denním reportu.",
    low: "Zaznamenat pro kontext.",
  };
  ...
  const alertData = {
    ...
    description: `Detekováno ${detectedSignals.length} signálů: ${detectedSignals.map(s => s.type).join(", ")}`,
    detected_signals: detectedSignals,
    recommended_action: actions[highestSeverity] || actions.medium,
    status: "new",
  ...
  try {
    ...
    const { data: alert, error } = await sbSafety.from("safety_alerts").insert(alertData).select("id").single();
    if (error) {
      console.error("[safety] Alert insert error:", error);
      return { safe: false, alert: alertData };
    }

    // For critical and high: send email notification
    if (highestSeverity === "critical" || highestSeverity === "high") {
      try {
        const emailUrl = `${Deno.env.get("SUPABASE_URL")}/functions/v1/send-safety-alert`;
        await fetch(emailUrl, {
          ...
          body: JSON.stringify({
            ...
            recommended_action: actions[highestSeverity],
          }),
        });
        await sbSafety.from("safety_alerts").update({
          notification_sent: true,
          notification_sent_at: new Date().toISOString(),
          notification_channel: "email",
        }).eq("id", alert.id);
      } catch (notifErr) {
        console.warn("[safety] Notification error:", notifErr);
      }
    }
    return { safe: false, alert: alertData };
  } catch (dbErr) {
    console.error("[safety] DB error:", dbErr);
    return { safe: false, alert: alertData };
  }
}
