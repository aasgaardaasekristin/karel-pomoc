...
const HanaChatInner = () => {
  ...
  return (
    <div className="relative flex flex-col h-full">
      ...
      {(() => {
        const portalTarget = document.getElementById("hana-header-actions");
        if (!portalTarget) return null;
        return createPortal(
          <Popover open={spravaOpen} onOpenChange={setSpravaOpen}>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className="h-8 px-2 text-xs gap-1 rounded-xl text-muted-foreground">
